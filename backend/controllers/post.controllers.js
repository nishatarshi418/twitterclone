...
export const deletePost = async (req,res) =>{
    try {
        ...
        console.log("Error is deletePost controllers:",error);
...
export const commentOnPost = async (req,res) =>{
  try {
    ...
    console.log("Error in commentOnPost controllers:",error);
...
export const likeUnlikePost = async (req,res) =>{
  try {
    ...
    console.log("Error in likeUnlikePost controllers:",error);
    res.status(500).json({error:"Internal server error"});
  }
};
...
export const getUserPosts = async(req,res) =>{
  try {
    ...
    if(!user) return res.status(404).json({error:"User not found"});
    const posts = await Post.find({user:user._id}).sort({createdAt}).populate({
      path:"user",
      select:"-password",
    }).populate({
      path:"comments.user",
